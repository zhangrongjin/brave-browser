...
console.log("utils.js: runGClient, args=", args);
//console.log("utils.js: runGClient, options=", options);
...
const util = {
  ...
  buildGClientConfig: () => {
console.log("util.js: buildGClientConfig 000 ")
    ...
    console.log("util.js: buildGClientConfig 003 solutions=", solutions)

    const out = 'solutions = ' + JSON.stringify(solutions, replacer, 2)
      .replace(/"%None%"/g, "None")
      .replace(/"%False%"/g, "False")
      + "\ntarget_os = " + JSON.stringify(target_os, replacer, 2);

console.log("util.js: buildGClientConfig 005 config.defaultGClientFile=", config.defaultGClientFile)
console.log("util.js: buildGClientConfig 005 out=", out)
  ...
  updateExtensionManifest: () => {
    console.log('update extensions manfiest...')
    const chromeDir = path.join(config.srcDir, 'chrome')
    const braveDir = path.join(config.projects.antimuon.dir)
    fs.copySync(path.join(braveDir, 'common' ,'extensions', 'api', 'brave_shields.json'), path.join(chromeDir, 'common', 'extensions', 'api', 'brave_shields.json'))
  },

  buildMuon: (options = config.defaultOptions) => {
    console.log('building brave...')

console.log('building brave... config.target=', config.target)

    const args = util.buildArgsToString(config.buildArgs())
    util.run('gn', ['gen', config.outputDir, '--args="' + args + '"'], options)
    util.run('ninja', ['-C', config.outputDir, '-j','16', /*'-v',*/ /*'brave'*/config.target], options)
  },
  ...
  // addAndroidTarget: (options = {}) => {
  //   ///util.run('fetch', ['--nohooks', 'android'], options)
  //   //echo "target_os = [ 'android' ]" >> ../.gclient
  //   util.run('echo', ["target_os = [ 'android' ]", ">> ../.gclient"], options);
  //   }

  //build/install-build-deps-android.sh
  installBuildAndroidDeps: (options = {}) => {
    console.log('Install and build dependencies for Android')
    util.run('src/build/install-build-deps-android.sh', [], options);
  }
}
