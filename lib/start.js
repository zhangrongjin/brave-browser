...
const start = (passthroughArgs, buildConfig = config.defaultBuildConfig, options) => {
  ...
  if (options.network_log) {
    let exitCode = 0
    let jsonOutput = {}
    // If the log is still being written, give it time to finish (10 seconds)
    console.log('Waiting for 10 seconds before reading "' + networkLogFile + '"')
    sleep(10)
    // Read the network log
    let jsonContent = fs.readFileSync(networkLogFile, 'utf8').trim()
    let jsonOriginal = jsonContent
